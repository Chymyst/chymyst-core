...
class JoinRunBlockingSpec extends FlatSpec with Matchers with TimeLimitedTests {
  ...
  it should "not block the entire threadpool when one thread is blocked" in {
    val c = ja[Unit]("c")
    val d = ja[Unit]("d")
    val g = js[Unit,Int]("g")
    val g2 = js[Unit,Int]("g2")
  ...
  it should "block the entire threadpool when one thread is blocked" in {
    val c = ja[Unit]("c")
    val d = ja[Unit]("d")
    val g = js[Unit,Int]("g")
    val g2 = js[Unit,Int]("g2")
    ...
    g2() shouldEqual 1 // this should initially work
    d() // do not inject c(). Now the first reaction is blocked because second reaction cannot start.
    g2(timeout = 100000000)() shouldEqual None // this should be blocked now
    tp.shutdownNow()
  }
  ...
  it should "block the fixed threadpool when one thread is sleeping with Thread.sleep" in {
    ...
    val tp = new ReactionPool(1)
    join(
      &{ case d(_) => Thread.sleep(500) } onThreads tp, // this thread is blocked by sleeping
      &{ case g2(_, r) => r(1) } onThreads tp // we will use this to test whether the entire thread pool is blocked
    )
    g2() shouldEqual 1 // this should initially work
  ...
  it should "not block the cached threadpool when one thread is sleeping with blocking(Thread.sleep)" in {
    val d = ja[Unit]("d")
    val g2 = js[Unit,Int]("g2")
    val tp = new CachedReactionPool(1)
    join(
