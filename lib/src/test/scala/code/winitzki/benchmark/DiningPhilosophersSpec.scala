...
class DiningPhilosophersSpec extends FlatSpec with Matchers with TimeLimitedTests {
  ...
  def diningPhilosophers(cycles: Int) = {
    ...
    site(tp, tp) (
      goSimple { case t1(n) => rw(h1); h1(n - 1) },
      goSimple { case t2(n) => rw(h2); h2(n - 1) },
      goSimple { case t3(n) => rw(h3); h3(n - 1) },
      goSimple { case t4(n) => rw(h4); h4(n - 1) },
      goSimple { case t5(n) => rw(h5); h5(n - 1) },

      goSimple { case done(_) + check(_, r) => r() },

      goSimple { case h1(n) + f12(_) + f51(_) => rw(t1); t1(n) + f12() + f51(); if (n == 0) done() },
      goSimple { case h2(n) + f23(_) + f12(_) => rw(t2); t2(n) + f23() + f12() },
      goSimple { case h3(n) + f34(_) + f23(_) => rw(t3); t3(n) + f34() + f23() },
      goSimple { case h4(n) + f45(_) + f34(_) => rw(t4); t4(n) + f45() + f34() },
      goSimple { case h5(n) + f51(_) + f45(_) => rw(t5); t5(n) + f51() + f45() }
    )
