...
- the user can set one-shot callbacks on per-molecule basis for unit testing.
...
The most verbose setting is the `DebugAllReporter`; the `ConsoleEmptyReporter` is completely silent.

Finally, a reaction pool should be created and assigned an event reporter.
By default, reaction pools are assigned the `ConsoleErrorReporter` that logs run-time errors to the console.

Here is sample code that defines the fully verbose console reporter and assigns it to a reaction pool:
